CREATE OR REPLACE PACKAGE BODY EmployeeNamesAndRowCounter AS
	...
	PROCEDURE LongestNameRecords IS
		CURSOR longest_name_cursor IS -- since there are multiple rows returning
			WITH employee AS (
		...
		cName VARCHAR2(40);
	BEGIN
		OPEN longest_name_cursor;
		LOOP
			FETCH longest_name_cursor INTO fName, lName, cName;
			EXIT WHEN longest_name_cursor%NOTFOUND;

			INSERT INTO LongestEmployeeNames (first_name, last_name, country) VALUES (fName, lName, cName);
		END LOOP;
		CLOSE longest_name_cursor;
